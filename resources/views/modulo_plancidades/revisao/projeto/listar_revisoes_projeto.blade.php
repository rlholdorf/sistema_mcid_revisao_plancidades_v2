...
<historico-navegacao :url="'{{ url('/home') }}'"
    :telanterior02="'Consultar Revisões de Projeto'"
    :link2="'{{url('/plancidades/revisao/projeto/consulta')}}'"
    :telanterior01="'PlanCidades'" 
    :link1="'{{url('/plancidades')}}'"
    :telatual="'Listar Revisões de Projeto'">
</historico-navegacao>


<div class="main-content pl-sm-3 mt-5 container-fluid" id="main-content">

    <cabecalho-relatorios :titulo="'Lista de Revisões de Projeto'" :linkcompartilhar="'{{ url('/') }}'" :barracompartilhar="false">
                    ...
                    <tbody>
                        @foreach ($revisoes as $revisao)
                            <tr class="conteudoTabela">
                                <td>{{ $revisao->revisao_projeto_id}}</td>
                                <td>{{ $revisao->txt_titulo_objetivo_estrategico_pei}}</td>
                                <td>{{ $revisao->projeto_id }} - {{ $revisao->txt_enunciado_projeto}}</td>
                                ...
                                <td class="acao" {{(($revisao->situacao_revisao_id == null ) || ($revisao->situacao_revisao_id == '1' )) ? 'disabled' : '' }}><a class="br-button circle primary small"
                                    href='{{ route("plancidades.revisao.projeto.show", ['revisaoId' => $revisao->revisao_projeto_id]) }}'><i
                                ...
                                <td class="acao" {{(($revisao->situacao_revisao_id == '3' ) || ($revisao->situacao_revisao_id == '5' ) || ($revisao->situacao_revisao_id == '6')) ? 'disabled' : '' }}><a class="br-button circle primary small"
                                    href='{{ route("plancidades.revisao.projeto.editar", ['revisaoId' => $revisao->revisao_projeto_id]) }}'><i
